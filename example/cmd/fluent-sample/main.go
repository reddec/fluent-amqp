...
func main() {
	...
	publisher := broker.Publisher().DefaultTopic("sample").DefaultKey("test").Create()

	requeue := broker.Requeue("test").Timeout(10 * time.Second).Create()

	broker.Sink("test").DeadLetter("", "deads").Retries(1).Lazy().Topic("sample").Key("*").TransactFunc(func(ctx context.Context, msg amqp.Delivery) error {
		println("gor")
